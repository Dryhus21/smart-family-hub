            ...
            {upcoming.map((e) => (
              <li key={e.id} className="card flex items-start gap-4">
                ...
                <div className="flex-1">
                  ...
                  <div className="mt-1 text-sm text-slate-600">{formatDateID(e.event_date)} {e.event_time && `· ${formatTimeID(e.event_time)}`}</div>
            ...
            {past.map((e) => (
              <li key={e.id} className="card flex items-center justify-between gap-3 opacity-70">
                <div>
                  ...
                  <div className="text-xs text-slate-500">{formatDateID(e.event_date)}</div>
                </div>
                {(ctx.userId === e.created_by || ctx.isAdmin) && (
                  <form action={deleteEventAction}>
                    <input type="hidden" name="id" value={e.id} />
